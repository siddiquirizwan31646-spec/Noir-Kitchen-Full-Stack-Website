...
export default function AuthCallback({ onSuccess }) {
  ...
  useEffect(() => {
    ...
    if (error || !token) {
      setStatus("error");
      setTimeout(() => { window.location.href = "/login?error=google_failed"; }, 2500);
      return;
    }
    ...
    fetch(`${API}/api/auth/me`, {
      ...
      .then((data) => {
        if (data.success) {
          // Save full user (including address) to localStorage
  ...
  return (
    <div style={styles.page}>
      ...
      <div style={styles.card}>
        ...
        {status === "verifying" && (
          <><Spinner /><p style={styles.heading}>Signing you in…</p><p style={styles.sub}>Verifying your Google account</p></>
        )}
        {status === "success" && (
          <><div style={styles.successIcon}>✓</div><p style={styles.heading}>Welcome back</p><p style={styles.sub}>Redirecting to your dashboard…</p></>
        )}
        {status === "error" && (
          <><div style={styles.errorIcon}>✕</div><p style={styles.heading}>Authentication failed</p><p style={styles.sub}>Redirecting you back to login…</p></>
        )}
      </div>
      ...
      <style>{`
        ...
        @keyframes pulse-ring { 0%{transform:translate(-50%,-50%) scale(0.85);opacity:0.5;}50%{transform:translate(-50%,-50%) scale(1.05);opacity:0.15;}100%{transform:translate(-50%,-50%) scale(0.85);opacity:0.5;} }
...
function Spinner() {
  return <div style={{ width:44,height:44,borderRadius:"50%",border:"2px solid rgba(196,81,10,0.2)",borderTopColor:"#C4510A",animation:"spin 0.9s linear infinite",margin:"0 auto 24px" }} />;
}
