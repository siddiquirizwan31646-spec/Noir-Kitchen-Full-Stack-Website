...
const SPICE_LEVELS = [
  { label: "Mild",   icon: "🌿", desc: "Gentle warmth, suitable for all palates",     color: "#4CAF50" },
  { label: "Medium", icon: "🌶", desc: "A pleasant heat that builds with each bite",   color: "#FF9800" },
  { label: "Hot",    icon: "🔥", desc: "Bold and fiery, recommended for spice lovers", color: "#F44336" },
  { label: "Fiery",  icon: "💥", desc: "Intense heat — a true chilli experience",      color: "#B71C1C" },
];
...
function Hero() {
  ...
  return (
    <section ref={ref} className="nkm-hero">
      <div className="nkm-spice-float nkm-sf1">✦</div>
      <div className="nkm-spice-float nkm-sf2">✦</div>
      <div className="nkm-leaf-float nkm-lf1">🌿</div>
      <div className="nkm-leaf-float nkm-lf2">🍃</div>
      <div className={`nkm-hero-text ${vis ? "nkm-fade-up" : "nkm-hidden"}`}>
        <p className="nkm-eyebrow">OUR MENU <span className="nkm-orn">✦</span></p>
        ...
        <div className="nkm-hero-btns">
          <button className="nkm-btn-primary" onClick={() => navigate("/signature-dishes")}>
...
<button className="nkm-btn-outline" onClick={() => navigate("/reserve")}>Reserve a Table</button>
      ...
      <div className={`nkm-hero-collage ${vis ? "nkm-fade-up nkm-delay-2" : "nkm-hidden"}`}>
        ...
        <div className="nkm-float-card nkm-fc1">⭐ Chef's Choice Tonight</div>
        <div className="nkm-float-card nkm-fc2">🏆 Michelin Inspired</div>
      </div>
    </section>
  );
}

/* ── MENU NAV — divided block cells ── */
      ...
      <div className="nkm-sig-body">
        ...
        <div className="nkm-sig-meta">
          ...
          <div className="nkm-sig-row">
            {item.prepTime && <span className="nkm-prep">🕐 {item.prepTime} min</span>}
            {item.rating   && <span className="nkm-rating">⭐ {item.rating}</span>}
          </div>
...
/* ── MENU ITEM — with round image top-left ── */
      ...
      <div className="nkm-item-content">
        <div className="nkm-item-left">
          <div className="nkm-item-name-row">
            ...
            {item.chef      && <span className="nkm-chef-badge">Chef's Pick</span>}
            {item.vegan     && <span className="nkm-vegan-tag">Vegan</span>}
...
/* ── CHEF PICKS — working section, round image top-left ── */
  ...
  return (
    <section className="nkm-section nkm-chefpicks" id="section-ChefPicks">
      <div ref={ref} className={`nkm-section-hd ${vis ? "nkm-fade-up" : "nkm-hidden"}`}>
        <p className="nkm-eyebrow">Curated Selections <span className="nkm-orn">✦</span></p>
  ...
  return (
    ...
    >
      ...
      <div className="nkm-pick-avatar-wrap">
        ...
        {!item.available && <div className="nkm-pick-avatar-unavail">✕</div>}
      </div>
  ...
  return (
    <section ref={ref} className="nkm-section nkm-spice-section">
      <div className={`nkm-section-hd ${vis ? "nkm-fade-up" : "nkm-hidden"}`}>
        <p className="nkm-eyebrow">Spice Guide <span className="nkm-orn">✦</span></p>
      ...
      <div className={`nkm-spice-cards ${vis ? "nkm-fade-up" : "nkm-hidden"}`}>
        {SPICE_LEVELS.map((s, i) => (
          <div key={s.label}
            ...
            <div className="nkm-spice-icon-wrap">
              <span className="nkm-spice-emoji">{s.icon}</span>
            </div>
            <h4 className="nkm-spice-label">{s.label}</h4>
            <p className="nkm-spice-desc">{s.desc}</p>
            <div className="nkm-spice-chilli-row">
              {[0,1,2,3].map(n => (
                <span key={n} className={`nkm-spice-chilli ${n <= i ? "nkm-spice-chilli-on" : ""}`}>🌶</span>
              ))}
            </div>
            ...
            <div className="nkm-spice-modal" onClick={e => e.stopPropagation()}>
              <div className="nkm-spice-modal-hd">
                <div className="nkm-spice-modal-hd-left">
                  <span className="nkm-spice-modal-hd-icon">{level.icon}</span>
                  <div>
                    <h3 className="nkm-spice-modal-title">{level.label} Dishes</h3>
                    <p className="nkm-spice-modal-subtitle">{level.desc}</p>
                  </div>
                </div>
                <button className="nkm-spice-modal-close" onClick={() => setSel(null)}>✕</button>
                <div className="nkm-spice-modal-hd-deco" aria-hidden>🌿</div>
              </div>
              {items.length === 0 ? (
                    ...
                    <div key={item._id}
                      ...
                      {item.chef && (
                        <div className="nkm-spice-modal-chef-badge">
                          <span>⭐</span><span>Chef's<br/>Pick</span>
                        </div>
                      )}
                      ...
                      <div className="nkm-spice-modal-right">
                        ...
                        <span className="nkm-spice-modal-spicy-tag">🌶 {level.label}</span>
                      </div>
                    </div>
                  ))}
                </div>
...
function CTA() {
  ...
  return (
    <section ref={ref} className="nkm-cta">
      <div className="nkm-cta-leaf nkm-cl1">🌿</div>
      <div className="nkm-cta-leaf nkm-cl2">🍃</div>
      <div className="nkm-cta-leaf nkm-cl3">✦</div>
      <div className={`nkm-cta-content ${vis ? "nkm-fade-up" : "nkm-hidden"}`}>
        <p className="nkm-eyebrow" style={{ justifyContent:"center", color:"#D86A1C" }}>Noir Kitchen <span className="nkm-orn">✦</span></p>
...
export default function NoirKitchenMenu({ user: propUser, onLogout, cart }) {
  ...
  useEffect(() => {
    const onScroll = () => {
      for (const id of [...NAV_TABS].reverse()) {
        const el = document.getElementById(`section-${id}`);
  ...
  if (loading) return (
    <div style={{ minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center", fontFamily:"Plus Jakarta Sans,sans-serif", color:"#D86A1C", fontSize:15 }}>
      Loading menu…
    </div>
  );
  if (error) return (
    <div style={{ minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center", fontFamily:"Plus Jakarta Sans,sans-serif", color:"#D32F2F", fontSize:15 }}>
  ...
  return (
    <>
      ...
      <div className="nkm-root">
        ...
        <Hero />
        <MenuNav active={activeTab} setActive={setActiveTab} />
        <main>
          {/* Signature */}
          <section id="section-Signature" className="nkm-section">
            <div className="nkm-section-hd">
              <p className="nkm-eyebrow">Chef's Collection <span className="nkm-orn">✦</span></p>
          ...
          {/* Chef Picks — placed right after Signature */}
          <ChefPicks items={chefPicks} navigate={navigate} user={user} cart={cart} />

          {/* Other categories */}
          {NAV_TABS.filter(t => t !== "Signature").map(tab => (
            menuData[tab] ? (
              <section key={tab} id={`section-${tab}`} className="nkm-section nkm-section-alt">
                <div className="nkm-section-hd">
                  <p className="nkm-eyebrow">{tab} <span className="nkm-orn">✦</span></p>
      ...
      <style>{`
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .nkm-orn { font-size: 12px; opacity: 0.7; }
        ...
        .nkm-prep { font-size: 11px; color: #9A8570; }
        .nkm-rating { font-size: 11px; color: #9A8570; font-weight: 600; }
        ...
        .nkm-add-btn { margin-top: 12px; width: 100%; padding: 9px; border-radius: 50px; border: 1.5px solid #D86A1C; background: transparent; color: #D86A1C; font-family: 'Plus Jakarta Sans',sans-serif; font-size: 12px; font-weight: 700; cursor: pointer; transition: all 0.2s; }
        ...
        .nkm-float-card { position: absolute; background: rgba(248,241,234,0.94); backdrop-filter: blur(12px); border: 1px solid rgba(216,106,28,0.2); border-radius: 14px; padding: 10px 16px; font-size: 12px; font-weight: 700; color: #1A1208; box-shadow: 0 8px 24px rgba(0,0,0,0.12); animation: nkmFloat 4s ease-in-out infinite; white-space: nowrap; }
        ...
        .nkm-leaf-float { position: absolute; font-size: 22px; opacity: 0.15; animation: nkmFloat 6s ease-in-out infinite; pointer-events: none; }
        .nkm-lf1 { top: 8%; left: 3%; animation-delay: 0.8s; }
        .nkm-lf2 { bottom: 10%; right: 3%; animation-delay: 2s; }

        /* ── NAV — divided block cells with gaps ── */
        .nkm-nav {
          position: sticky;
          top: 80px;
          z-index: 900;
        ...
        .nkm-nav-tab:hover {
          background: rgba(216,106,28,0.07);
          color: #D86A1C;
          border-color: rgba(216,106,28,0.25);
          transform: translateY(-1px);
        }
        .nkm-nav-active {
          color: #D86A1C;
          background: rgba(216,106,28,0.1);
          border-color: rgba(216,106,28,0.4);
          box-shadow: 0 2px 8px rgba(216,106,28,0.15);
        }
        .nkm-nav-active::after {
          content: '';
          position: absolute;
          bottom: -1px;
          left: 50%;
        ...
        .nkm-sig-card { background: #fff; border-radius: 20px; overflow: hidden; box-shadow: 0 6px 28px rgba(0,0,0,0.08); border: 1px solid rgba(216,106,28,0.1); transition: transform 0.3s,box-shadow 0.3s; }
        .nkm-sig-card:hover { transform: translateY(-6px); box-shadow: 0 18px 48px rgba(216,106,28,0.18); }
        .nkm-sig-img-wrap { position: relative; height: 220px; overflow: hidden; }
        ...
        .nkm-sig-body { padding: 20px; }
        .nkm-sig-top { display: flex; align-items: flex-start; justify-content: space-between; gap: 12px; margin-bottom: 10px; }
        .nkm-sig-name { font-family: 'Cormorant Garamond',serif; font-size: 20px; font-weight: 600; color: #1A1208; line-height: 1.2; }
        .nkm-sig-price { font-family: 'Cormorant Garamond',serif; font-size: 22px; font-weight: 600; color: #D86A1C; white-space: nowrap; }
        .nkm-sig-desc { font-size: 13px; color: #6B5B45; line-height: 1.7; margin-bottom: 14px; }
        .nkm-sig-meta { display: flex; flex-direction: column; gap: 8px; }
        .nkm-sig-pairing { font-size: 11px; color: #9A8570; font-style: italic; }

        /* ── MENU ITEMS — with round avatar ── */
        ...
        /* Round image top-left on MenuItem */
        .nkm-item-avatar-wrap {
          flex-shrink: 0;
          width: 62px;
          height: 62px;
          border-radius: 50%;
        ...
        .nkm-item-content { display: flex; align-items: flex-start; justify-content: space-between; gap: 12px; flex: 1; min-width: 0; }
        .nkm-item-left { flex: 1; min-width: 0; }
        .nkm-item-name-row { display: flex; align-items: center; gap: 7px; flex-wrap: wrap; margin-bottom: 5px; }
        .nkm-item-name { font-family: 'Cormorant Garamond',serif; font-size: 17px; font-weight: 600; color: #1A1208; line-height: 1.2; }
        .nkm-chef-badge { font-size: 9px; font-weight: 700; letter-spacing: 1px; text-transform: uppercase; color: #D86A1C; background: rgba(216,106,28,0.1); border-radius: 12px; padding: 2px 8px; white-space: nowrap; }
        .nkm-item-desc { font-size: 12.5px; color: #6B5B45; line-height: 1.65; margin-bottom: 7px; }
        .nkm-item-ing { font-size: 11px; color: #9A8570; line-height: 1.5; margin-bottom: 7px; }
        ...
        .nkm-pick-avatar-wrap {
          flex-shrink: 0;
          width: 72px;
          height: 72px;
          border-radius: 50%;
          overflow: hidden;
          border: 3px solid rgba(216,106,28,0.2);
        ...
        .nkm-pick-body { flex: 1; min-width: 0; }
        ...
        .nkm-pick-name { font-family: 'Cormorant Garamond',serif; font-size: 18px; font-weight: 600; color: #1A1208; line-height: 1.2; }
        ...
        .nkm-pick-desc { font-size: 12px; color: #6B5B45; line-height: 1.6; margin-bottom: 10px; }
        .nkm-pick-footer { display: flex; align-items: center; gap: 10px; flex-wrap: wrap; margin-bottom: 4px; }
        ...
        .nkm-spice-icon-wrap { width: 72px; height: 72px; border-radius: 50%; background: rgba(255,255,255,0.9); border: 1.5px solid rgba(0,0,0,0.06); display: flex; align-items: center; justify-content: center; box-shadow: 0 4px 14px rgba(0,0,0,0.08); transition: transform 0.3s ease; }
        .nkm-spice-card:hover .nkm-spice-icon-wrap { transform: scale(1.1) rotate(-5deg); }
        .nkm-spice-emoji { font-size: 34px; line-height: 1; }
        ...
        .nkm-spice-chilli { font-size: 16px; opacity: 0.15; transition: opacity 0.2s, transform 0.2s; }
        ...
        .nkm-spice-modal-hd-icon { font-size: 36px; line-height: 1; }
        ...
        .nkm-spice-modal-hd-deco { position: absolute; right: 60px; top: -10px; font-size: 80px; opacity: 0.07; pointer-events: none; transform: rotate(20deg); }
        ...
        .nkm-spice-modal-spicy-tag { font-size: 11px; font-weight: 600; color: #D86A1C; background: rgba(216,106,28,0.1); border-radius: 20px; padding: 3px 10px; white-space: nowrap; }
        ...
        .nkm-cta-leaf { position: absolute; font-size: 48px; opacity: 0.06; animation: nkmFloat 6s ease-in-out infinite; pointer-events: none; }
        .nkm-cl1 { top: 10%; left: 5%; }
        .nkm-cl2 { bottom: 10%; right: 5%; animation-delay: 1s; }
        .nkm-cl3 { top: 40%; left: 50%; font-size: 80px; color: #D86A1C; animation-delay: 2s; }
        ...
        @media (max-width:900px) {
          ...
          .nkm-nav { top: 72px; }
          .nkm-items-grid { grid-template-columns: 1fr; }
          .nkm-sig-grid { grid-template-columns: repeat(auto-fill,minmax(240px,1fr)); }
          .nkm-picks-grid { grid-template-columns: 1fr; }
        }
        ...
        @media (max-width:600px) {
          ...
          .nkm-items-grid { gap: 10px; }
          .nkm-item { padding: 14px 12px; gap: 10px; }
          .nkm-item-avatar-wrap { width: 52px; height: 52px; }
          .nkm-pick-card { flex-direction: row; flex-wrap: wrap; }
          .nkm-pick-avatar-wrap { width: 54px; height: 54px; margin-top: 0; }
          .nkm-picks-grid { gap: 12px; }
          .nkm-spice-modal-overlay { padding: 0; align-items: flex-end; }
          .nkm-spice-modal { border-radius: 20px 20px 0 0; max-height: 90vh; }
          .nkm-section-hd { margin-bottom: 36px; }
        }
        @media (max-width:400px) {
          .nkm-nav-tab { padding: 7px 7px; font-size: 9.5px; border-radius: 5px; }
          .nkm-spice-cards { grid-template-columns: 1fr 1fr; gap: 8px; }
          .nkm-item-avatar-wrap { width: 46px; height: 46px; }
        }
